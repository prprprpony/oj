...
int main()
{
	...
	sort(p,p+n,[](po a, po b)->bool{return a.x < b.x;});
	REP(i,1,n)
		v.EB(p[i].x-p[i-1].x,pii{p[i].i,p[i-1].i});
	sort(p,p+n,[](po a, po b)->bool{return a.y < b.y;});
	REP(i,1,n)
		v.EB(p[i].y-p[i-1].y,pii{p[i].i,p[i-1].i});
	ll ans = 0;
	sort(ALL(v));
	RP(i,n) g[i] = i;
	for (auto e : v) {
