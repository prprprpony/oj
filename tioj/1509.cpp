...
/* Good Luck && Have Fun ! */
const int N = 1e6 + 87,inf = 1.0087e9;
pair<int,pii> g[N];
int d[N],q[N],f[N],l,r,sz,n,m;
bool inq[N];
void pb(int x)
{
	inq[x] = 1;
...
int main()
{
	...
	blg();
	ll ans = label_correcting_algorithm();
	RP(i,m)
		swap(g[i].F, g[i].S.F);
	blg();
	ans += label_correcting_algorithm();
	PL( ans );
}
