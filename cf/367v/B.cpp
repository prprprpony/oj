...
int main()
{
	...
	val.reserve(m);
	RP(i,n) RD(a[i]);
	RP(i,m) {RD(b[i]); val.PB(b[i]);}
	sort(ALL(val));
	val.erase(unique(ALL(val)),val.end());
	RP(i,n) {
		int k = lower_bound(ALL(val),a[i]) - val.begin();
		if (k < SZ(val) && val[k] == a[i])
			a[i] = k;
		else
			a[i] = SZ(val);
	}
	RP(i,m) ++c[b[i] = lower_bound(ALL(val),b[i]) - val.begin()];
	vi ans;
	RP(i,p) {
		int same = 1, j, k;
