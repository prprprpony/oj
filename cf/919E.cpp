...
int inv(int a,int m) {return qpow(a,m-2,m);}
int _div(int a,int b,int m) {return mul(a,inv(b,m),m);}
int sub(int a,int b,int m) {return (a -= b) < 0 ? a + m : a;}
int main()
{
    RI(a,b,p);
    DR(ll,x);
    ...
    int iw = inv(w,p), ia = inv(a,p);
    RP1(i,min(ll(w),x)) {
        b = mul(b,ia,p);
        int t = mul(sub(b,i,p),iw,p);
        ll r = (x-i)/w;
        ans += r/p + (r%p >= t);
        // wy+i <= x
        // y <= (x-i)/w
