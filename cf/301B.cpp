...
ll dis(int i,int j)
{
	return r * (abs(p[i].F-p[j].F) + abs(p[i].S-p[j].S)) - a[i];
}
/*
	dis(i,j) >= 0  
	because no two stations are located at the same point
	and coordinates are integer
	and r >= 1000
	and a[i] <= 1000

	since dis(i,j) >= 0
	there are no negative edge

	since there are no negative edge
	there are no negative cycle

	PERHAPS THE GREATEST TASK EVER!!!
*/
int main()
{
	RD(n,r);
	REP1(i,2,n-1) RD(a[i]);
	RP1(i,n) RD(p[i].F,p[i].S);
	fill_n(d,n+1,inf);
	d[1] = 0;
