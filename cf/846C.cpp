...
int main()
{
	...
	PR(i,n) p[i] += p[i+1];
	ml[0] = 0;
	REP1(i,1,n) ml[i] = p[i] < p[ml[i-1]] ? i : ml[i-1];
	mr[n] = n;
	PER1(i,n-1,0) mr[i] = p[i] < p[mr[i+1]] ? i : mr[i+1];
	pll ans(p[0]-p[ml[0]]-p[mr[0]], 0);
	REP1(i,1,n) cmax(ans, pll(p[i]-p[ml[i]]-p[mr[i]], i));
	int j = ans.S;
	PL(ml[j],j,mr[j]);
}
