...
int main()
{
	...
	RP1(i,n) sl[i] += sl[i-1];
	RP1(i,m) st[i] += st[i-1];
	PR1(i,n-1) sr[i] += sr[i+1];
	PR1(i,m-1) sb[i] += sb[i+1];
	RIIII(cl,cr,ct,cb);
	RP(i,d) {
		if (
		sl[x[i].S-1] - (x[i].F < x[i].S) == cl &&
		sr[x[i].F+1] - (x[i].S > x[i].F) == cr &&
		st[y[i].S-1] - (y[i].F < y[i].S) == ct &&
		sb[y[i].F+1] - (y[i].S > y[i].F) == cb
		) {
			PL(i+1);
			return 0;
		}
