...
const int N = 1e5 + 87;
int dp[2][3][2];
void upd(int a[2],int b[2],int x, bool in = 1)
{
    if (in)
        fill_n(a,2,0);
    RP(i,2) cmax(a[i],b[i]);
...
int main()
{
    ...
    RP(i,n) {
        ...
        DR(char,c);
        int x = c - '0';
        upd(dp[k][0],dp[k^1][0],x);
        upd(dp[k][1],dp[k^1][0],x^1);
        upd(dp[k][1],dp[k^1][1],x^1,0);
        upd(dp[k][2],dp[k^1][2],x);
        upd(dp[k][2],dp[k^1][1],x,0);
    }
    int ans = 0;
    RP(i,3)
        RP(j,2)
            cmax(ans, dp[n&1^1][i][j]);
    PL(ans);
}
