...
/* Good Luck && Have Fun ! */
const int N = 1e5 + 87, inf = 1e9 + 1;
struct rin
{
	int a,b,h;
	bool operator < (const rin & x) const 
	{
...
ll qry(node * t, int l, int r,int i,int j)
{
	...
	if (i <= l && r <= j)
		return t->mx;
	int m = l + ((r - l)>>1);
	return max(qry(t->lc, l, m, i ,j), qry(t->rc, m, r, i ,j));
}
void upd(node *& t, int l, int r, int i, ll v)
{
	if (!t)
		t = new node();
	cmax(t->mx, v);
...
int main()
{
	DRI(n);
	RP(i,n)
		RD(r[i].a, r[i].b, r[i].h);
	sort(r,r+n);
	RP(i,n) 
		upd(rt,1,inf,r[i].a,qry(rt,1,inf,1,r[i].b) + r[i].h);
	PL(rt->mx);
}
