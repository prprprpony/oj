...
int main()
{
    ...
    RP(i,n) {
        RI(a,b);
        dp[i&1].clear();
        for (auto it = begin(dp[i&1^1]); it != end(dp[i&1^1]); ++it) {
            int s = it->F;
            if (dec(s,v) == 5)
                continue;
            int j = s & 15;
            s >>= 4;
            upd(i&1, (s << 4 | b) << 4 | a , it->S + abs(j - a));
        }
        for (auto it = begin(dp[i&1]); it != end(dp[i&1]); ) {
            int w = dec(it->F, v);   
            int s = it->F;
            int j = v[0];
            REP(k,1,w) {
                upd(i&1, (rmv(s, k) >> 4) << 4 | v[k] , it->S + abs(j - v[k]));
            }
            ++it;
        }
    }
