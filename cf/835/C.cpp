...
struct qqq
{
	...
	int t;
	int i;
	bool operator < (const qqq & rt) const
	{
		return x < rt.x;
	}
} b[N*2];
int ans[N],sg[101 * 4][11],ret[11];
void add(int o,int l,int r,int i,int s)
{
	sg[o][s]++;
	if (r - l == 1)
		return;
...
void qry(int o,int l,int r,int i,int j)
{
	if (r <= i || j <= l)
		return;
	if (i <= l && r <= j) {
...
int main()
{
	RD(n,q,c);
	++c;
	RP(i,n) RD(a[i].F.F,a[i].F.S,a[i].S);
	sort(a,a+n);
	RP(i,q) {
		RIII(t,x1,y1);
		RII(x2,y2);
		b[i<<1] = {x1-1,{y1,y2},t,i<<1};
		b[i<<1|1] = {x2,{y1,y2},t,i<<1|1};
	}
	sort(b,b+2*q);
	int i = 0, j = 0;
	REP1(x,0,100) {
		for (; i < n && a[i].F.F == x; ++i)
			add(1,1,101,a[i].F.S,a[i].S);
		for (; j < 2*q && b[j].x == x; ++j) {
			fill_n(ret,c,0);
			qry(1,1,101,b[j].y.F,b[j].y.S+1);
			int t = 0;
			RP(k,c) {
				int s = (k + b[j].t) % c;
				t += ret[k] * s;
			}
			ans[b[j].i>>1] += ((b[j].i&1)?1:-1) * t;
		}
	}
	RP(i,q) PL(ans[i]);
}
