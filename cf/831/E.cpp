...
struct node
{
	int s,v,mn,pri;
	node * lc, * rc;
	node(int x) : s(1), v(x), mn(x), pri(ran()), lc(nullptr), rc(nullptr) {}
	void pull()
	{
		s = 1;
		mn = v;
...
void split_mn(node * t,node *& a,node *&b)
{
	if (!t) {
		a = b = nullptr;
		return;
	}
	if (t->lc&&t->lc->mn==t->mn) {
		b = t;
		split_mn(t->lc,a,b->lc);
		b->pull();
	} else if (t->v == t->mn) {
		a = t;
		b = t->rc;
