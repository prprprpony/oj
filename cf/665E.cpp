...
void add(node *& t,int v,int b)
{
	...
	if (!b)
		return;
	add(t->c[(v&b) ? 1 : 0],v,b>>1);
}
int sz(node * t) {return t ? t->s : 0;}
int qry(node * t,int k,int v,int b)
{
	if (!t || !b) return sz(t);
	return qry(t->c[((k&b)^(v&b)) ? 1 : 0],k,v,b>>1) + (!(k&b) ? sz(t->c[((v&b) ^ b) ? 1 : 0]) : 0);
}
int main()
{
	DRII(n,k);
