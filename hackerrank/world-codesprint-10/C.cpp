...
/* Good Luck && Have Fun ! */
const int N = 1e5+87, D = 60, M = 1e9+7;
int f[N],fi[N];
ll a[N];
void pre(int n)
{
	++n;
	f[0] = f[1] = 1;
	REP(i,2,n) f[i] = f[i-1] * 1LL * i % M;
	fi[0] = fi[1] = 1;
	REP(i,2,n) fi[i] = fi[M%i] * 1LL * (M-M/i) % M;
	REP(i,2,n) fi[i] = fi[i] * 1LL * fi[i-1] % M;
/*
M%i = - M/i*i
i^(-1) = (M%i)^(-1) * -(M/i)
*/
}
tuple<int,ll> solve(int n,int k,int d,ll v)
{
	if (d < 0)
		return {n,v};
	int i = 0,j = n-1;
	while (i <= j)
		if (a[i] & (1LL << d))
			...
			swap(a[i],a[j--]);
	return solve(i >= k ? i : n,k,d-1, v | ((1LL<<d) * (i >= k)));
}
int main()
{
	RII(n,k);
	...
	int m;
	ll v;
	tie(m,v) = solve(n,k,D,0);
	PL(v);
	pre(m);
	PL(f[m] * 1LL * fi[k] % M * fi[m-k] % M);
}
